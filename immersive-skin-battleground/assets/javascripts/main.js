...
$(document).ready(function() {
	...
	$('#search_category').on('change', function(){
		...
		var child = $(this).find('option:selected').attr('data-child-list').split(',');
		for (var i in child)
		{
			if (child[i] != '-1')
			{
...
function stickSidebarLanding(){
	...
	if( sidebar.height() < landingMainBar.height() ){
		if( windowPanel.scrollTop() > ( heightright - windowPanel.height() ) + batasScrollBawah.height() + sliderHeight + 10 ){
...
function kstore_widget() {
	...
	$('#product_id').change(function(){
		for(i in products)
		{
			if(products[i].id == $(this).val())
			{
				$('#harga_epulsa').html(products[i].price.toLocaleString('id-ID', {style: "currency", currency: "IDR"}).replace('Rp', 'Rp '));
				break;
			}
		}
	});
	...
	$('#nominal_pln').change(function(){
		for(i in pln_products)
		{
			...
			{
				$('#harga_token').html(pln_products[i].price.toLocaleString('id-ID', {style: "currency", currency: "IDR"}).replace('Rp', 'Rp '));
				break;
			}
		}
	});
	...
	$('#phone').keyup(function(){
		...
		if(value.length >= 4)
		{
			currentPrefix = value.substring(0, 4);
			if(prevPrefix != currentPrefix && prefixOperator[currentPrefix])
			{
				$('#operator_pulsa').val(prefixOperator[currentPrefix]);
				$.getJSON(
					EPULSA_URL + '/api/v1/operators/' + prefixOperator[currentPrefix] + '?callback=?',
					function(data){
						$('#product_id').html('<option value="" disabled>Pilih paket</option>');
	...
	$.getJSON(
		EPULSA_URL + '/api/v1/operators/17?callback=?',
		function(data){
			$('#nominal_pln').html('<option value="" disabled>Pilih paket</option>');
			pln_products = data.products.data;
			for(i in pln_products)
			{
				$('<option value="' + pln_products[i].id + '">' + pln_products[i].name + '</option>')
				.appendTo('#nominal_pln');
			}
			$('#nominal_pln').change();
		}
	);
